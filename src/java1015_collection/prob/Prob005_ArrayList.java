...
public class Prob005_ArrayList {
	...
	public static void main(String[] args) {
      ...
      */
		File file = new File("src/java1015_collection/prob/tv.txt");
		try {
			...
			while(sc.hasNextLine()) {
				...
				tv.setRank(Integer.parseInt(stn[0]));
				tv.setProgram(stn[1]);
				tv.setChannel(stn[2]);
				tv.setRating(Double.parseDouble(stn[3]));
	...
	public static void prnDisplay(ArrayList<Television> aList, String channel){
		...
		for(int i = 0; i < aList.size(); i++) {
			if(aList.get(i).getChannel().equals(channel))
				System.out.println(aList.get(i).getRank() + "   " + aList.get(i).getProgram() + "   " + aList.get(i).getChannel() + "   " + aList.get(i).getRating() + "   ");
		}
	}//end prnDisplay()
